...
with open("ghIn_W1", newline="") as fin:
    reader = csv.DictReader(fin)
    symbols = [row["T"] for row in reader]

with open("ghOut_W1", "w", newline="", encoding="utf-8") as fout:
    ...
    for sym in symbols:
        ...
        for attempt in range(4):
            try:
                ...
                if attempt == 0:
                    wait = random.uniform(3.6,3.9)
                elif attempt == 1:
                    wait = random.uniform(5.4,5.7)
                elif attempt == 2:
                    wait = random.uniform(7.2,7.5)
                else:
                    break
                time.sleep(wait)
